...
export function ServiceCard({ service }: ServiceCardProps) {
  ...
  return (
    <a
      ...
      className="group flex flex-col items-center gap-3 p-4 rounded-xl border border-(--border) bg-(--card) hover:border-(--dim) hover:bg-[#111d2e] transition-all duration-150"
    ...
    >
      ...
      <div className="flex flex-col items-center gap-1 min-w-0 w-full">
        ...
        {service.shortcut && (
          <kbd className="text-[9px] text-(--text-muted) font-mono tracking-widest uppercase">
            {service.shortcut}
          </kbd>
        )}
      </div>
